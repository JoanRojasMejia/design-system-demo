...
import type { Component, ComputedRef, Ref, SetupContext, WritableComputedRef } from 'vue'

type propsUseSelect = ISelectProps & { slots: SetupContext['slots'] }

type useSelectReturnType = (
  props: propsUseSelect,
  emit: SelectEmitFn
) => {
  ...
  helpTextKls: ComputedRef<(string | Record<string, boolean>)[]>
...
const useSelect: useSelectReturnType = (props: propsUseSelect, emit: SelectEmitFn) => {
  // inject
  const { t } = useLocale()
  ...
  const { aliasProps, getTitle, getValue, getDisabled, getOptions } = useProps(props)
  ...
  const { isComposing, handleCompositionStart, handleCompositionEnd, handleCompositionUpdate } =
    useComposition({
  ...
  const iconComponent = computed(() =>
    props.remote && props.filterable ? '' : 'regular chevron-down'
  )
  ...
  const isErrorKls = computed(() => Boolean(selectError.value || elFormItem?.shouldShowError))
  const helpTextKls = computed(() => [
    nsSelect.e('help-text'),
    {
      [nsSelect.e('help-error')]: selectError.value
    }
  ])
  ...
  const calculateLabelMaxWidth = () => {
    ...
    const maxWidth = filteredOptions.value.reduce((max, option) => {
      const metrics = ctx.measureText(getTitle(option))
  ...
  const onSelect = (option: Option) => {
    if (props.multiple) {
      ...
      states.selectedLabel = getTitle(option)
  ...
  const getLastNotDisabledIndex = (value: unknown[]) =>
    findLastIndex(
      value,
      (it) => !states.cachedOptions.some((option) => getValue(option) === it && getDisabled(option))
    )
  ...
  const updatePrefixPosition = () => {
    ...
    requestAnimationFrame(() => {
      ...
      leftPrefixSelect.value = `${leftRef}px`
    })
  }
    ...
    if (props.multiple) {
      ...
      if (hasModelValue.value) {
        ...
        if (~selectedItemIndex) {
          states.selectedLabel = getTitle(options[selectedItemIndex])
        } else {
  ...
  watch(
    ...
    (val, oldVal) => {
      ...
      if (
        ...
        (!props.multiple && getValueKey(val) !== getValueKey(states.previousValue))
      ) {
        initStates(true)
      }
